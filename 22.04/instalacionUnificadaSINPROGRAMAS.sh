...
if [ "$opcionProfeAlum" == "1" ]; then
	...
	sudo usermod -aG vboxusers,dialout,epoptes,profesorado $USER

	# Configurar pam para agregar usuarios a grupos al inicio
	echo "*;*;*;Al0000-2400;users,dialout,vboxusers,epoptes,profesorado" | sudo tee -a /etc/security/group.conf
